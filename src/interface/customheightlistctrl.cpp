...
void wxCustomHeightListCtrl::OnDraw(wxDC& dc)
{
	...
	// Use the theme's window-text colour for the focus outline so it stays
	// visible against both light and dark backgrounds.
	wxColour const focusPenColour = wxSystemSettings::GetColour(wxSYS_COLOUR_WINDOWTEXT);

	for (auto const& selected : m_selectedLines) {
		if (selected == m_focusedLine) {
			dc.SetPen(wxPen(focusPenColour, 1, wxPENSTYLE_DOT));
		}
		else {
			dc.SetPen(*wxTRANSPARENT_PEN);
		}
